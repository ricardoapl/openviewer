...
export default {
  ...
  methods: {
    selectCluster: function () {
      let url = ''
      if (this.cluster.ip === '0.0.0.0') {
        url = 'https://127.0.0.1:' + this.cluster.port
      } else {
        url = 'https://' + this.cluster.ip + ':' + this.cluster.port
      }
      const mutation = 'clusters/setClusterServer'
      this.$store.commit(mutation, url)
